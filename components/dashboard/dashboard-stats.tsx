...
export async function DashboardStats() {
  ...
  const cards = [
    ...
    },
    {
      ...
      sub: stats.pendingMatches > 0 ? `${stats.pendingMatches} ausstehend` : undefined,
      icon: TrendingUp,
    ...
    },
    {
      ...
      sub: stats.submittedApplications > 0
        ? `${stats.submittedApplications} eingereicht`
        : undefined,
      icon: Send,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map(({ label, value, sub, icon: Icon, color, bg, href }) => (
          <Link
            key={label}
            href={href}
            className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex items-center gap-4 hover:border-slate-600 transition-colors"
          >
        ...
        <div className="grid grid-cols-2 gap-4">
          {stats.interviewCount > 0 && (
            <div className="bg-emerald-900/10 border border-emerald-800/40 rounded-xl p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-semibold">{stats.interviewCount} Interview{stats.interviewCount > 1 ? "s" : ""}</p>
                <p className="text-emerald-400/70 text-xs">Aktive Bewerbungen</p>
              </div>
            </div>
          )}
          {stats.offerCount > 0 && (
            <div className="bg-yellow-900/10 border border-yellow-800/40 rounded-xl p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-semibold">{stats.offerCount} Angebot{stats.offerCount > 1 ? "e" : ""}</p>
                <p className="text-yellow-400/70 text-xs">Herzlichen Glückwunsch!</p>
              </div>
            </div>
          )}
        </div>
